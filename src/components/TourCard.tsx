...
export function TourCard({ tour, revealDelay = 0 }: TourCardProps) {
  ...
  return (
    <MotionReveal delay={revealDelay}>
      <Link
        to={`/tours/${tour.slug}`}
        className="group relative block overflow-visible rounded-[1.75rem] border border-[#eadfd3] bg-white pb-0 shadow-[0_18px_42px_rgba(17,24,39,0.08)] transition duration-300 hover:-translate-y-1 hover:border-[#decfc0] hover:shadow-[0_24px_55px_rgba(17,24,39,0.12)]"
      >
        ...
        <div className="relative z-10 -mt-8 flex min-h-60 flex-col rounded-t-[1.75rem] rounded-b-[1.75rem] bg-white px-7 pb-7 pt-9">
          <h3 className="text-2xl font-medium leading-tight text-ink transition group-hover:text-[#FD5E02]">{title}</h3>
          <p className="tour-description mt-4 flex-1 text-lg leading-8 text-muted">{shortDescription}</p>
          <div className="mt-6 flex items-center justify-between gap-4">
            <span className="inline-flex items-center justify-center gap-2 rounded-full bg-[#263238] px-4 py-2.5 text-xs font-bold uppercase tracking-[0.08em] text-white transition group-hover:bg-[#111827]">
