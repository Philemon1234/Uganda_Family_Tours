...
function phoneHrefFromDisplay(phone: string) {
...
export function Footer({ customization }: { customization: HomeCustomizationContent }) {
    ...
    <footer id="site-footer" className="relative bg-dark text-white">
      <div className="container-custom grid grid-cols-2 gap-x-6 gap-y-7 pb-8 pt-8 md:grid-cols-2 md:gap-10 md:py-14 lg:grid-cols-[1.3fr_1fr_1fr_1fr]">
        <div className="col-span-2 md:col-span-1">
          <Logo />
          <p className="mt-3 max-w-sm text-xs leading-5 text-white/65 md:mt-5 md:text-sm md:leading-7">{customization.footer.about || t('footer.about')}</p>
        </div>
        <div className="col-span-2 md:col-span-1">
          ...
          <ul className="mt-3 grid gap-2 text-sm text-white/75 sm:grid-cols-3 md:mt-5 md:block md:space-y-3">
            ...
            <li><a className="flex min-w-0 gap-2 transition hover:text-primary md:gap-3" href={emailHref}><MdEmail className="mt-1 shrink-0 text-primary" /><span className="truncate">{customization.footer.email}</span></a></li>
            <li className="flex min-w-0 gap-2 md:gap-3"><FaLocationDot className="mt-1 shrink-0 text-primary" /><span className="truncate">{customization.footer.location || t('footer.location')}</span></li>
          </ul>
        </div>
        <div>
          <h3 className="footer-title">{t('footer.quickLinks')}</h3>
          <ul className="mt-3 space-y-1.5 text-sm text-white/75 md:mt-5 md:space-y-2">
            {customization.footer.quickLinks.map((item) => (
              <li key={item.id}><a className="transition hover:text-primary" href={item.href}>{item.label}</a></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <div className="mt-3 flex gap-3 md:mt-4">
            {socials.map(({ labelKey, icon: Icon }, index) => {
              const social = customization.footer.socials[index]
              return (
              <a key={labelKey} className="grid h-9 w-9 place-items-center rounded-full border border-white/20 transition hover:border-primary hover:text-primary" href={social?.href || '#'} target="_blank" rel="noreferrer" aria-label={social?.label || t(labelKey)}>
                <Icon />
              </a>
              )
            })}
