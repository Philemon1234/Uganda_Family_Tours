...
type WhyTravelCardProps = {
  ...
  title: string
  text: string
  iconImage?: EditableImage
  variant?: 'light' | 'dark'
  revealDelay?: number
}

export function WhyTravelCard({ icon: Icon, iconImage, title, text, variant = 'light', revealDelay = 0 }: WhyTravelCardProps) {
  ...
  return (
    <MotionReveal delay={revealDelay}>
      <div className={isDark ? 'rounded-3xl border border-white/10 bg-white/[0.04] px-3 py-6 text-center sm:px-6 sm:py-8' : 'text-center'}>
        <div className={isDark ? 'mx-auto grid h-14 w-14 place-items-center rounded-full bg-primary text-xl text-ink sm:h-16 sm:w-16 sm:text-2xl' : 'mx-auto grid h-16 w-16 place-items-center rounded-full bg-primary text-2xl text-ink shadow-orange'}>
          {iconImage?.src ? <img src={iconImage.src} alt={iconImage.alt} className="h-7 w-7 object-contain" /> : <Icon />}
        </div>
